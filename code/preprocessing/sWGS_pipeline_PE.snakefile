...
R1IDS, = glob_wildcards("demultiplexed_reads/{sample}_R1.fastq.gz")
...
rule mapping_bwa:
    input:
        fq1 = "demultiplexed_reads/{sample}_R1.fastq.gz"
        fq2 = "demultiplexed_reads/{sample}_R2.fastq.gz"
    output:
    ...
    shell:
        ...
        "bwa mem -t {threads} {params.genome} {input.fq1} {input.fq2} | samtools sort -O bam -o {output}"
...
rule multiqc:
    input:
        expand("logs/{sample}_samtools.stats", sample = R1IDS),
        expand(directory("ichorCNA_low/{sample}/"), sample = R1IDS),
        expand(directory("ichorCNA_high/{sample}/"), sample = R1IDS),
        expand("plotnpz/{sample}_bins.bed", sample = R1IDS)
    output:
